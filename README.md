...
- کنترل start/stop/restart سرویس‌های Bot و Web Panel از داخل Web Panel
- تغییر امن BOT_TOKEN، ADMIN_IDS و رمز Web Panel بدون نمایش مقدارهای فعلی
- حذف سرویس Bot بدون حذف Web Panel یا دیتابیس
...
## امنیت کنترل سرویس‌ها

- `aval-bot-web` فقط از طریق helper محدود systemd سرویس‌ها را کنترل می‌کند.
- در صفحهٔ تنظیمات، Token و رمز فقط وضعیت «تنظیم شده (مخفی)» نشان می‌دهند؛ مقدار واقعی هیچ‌وقت نمایش داده نمی‌شود.
- دکمهٔ «نمایش رمز» فقط برای مقدار جدیدی است که همان لحظه در فرم وارد می‌کنی؛ مقدار ذخیره‌شده قابل مشاهده نیست.
- دکمهٔ حذف Bot فقط unit مربوط به Bot را حذف می‌کند و Web Panel، `.env` و `bot.db` را نگه می‌دارد.
